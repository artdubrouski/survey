...
def test_user_id_created(api_admin, survey_active, api_user):
    ...
    user_got = api_user.post('/api/v1/survey-responses/', data=survey_response)
...
def test_user_cookie_created(api_admin, survey_active):
    ...
    survey_response = get_survey_response(admin_got['pk'])
    api_user = APIClient()
    response = api_user.post('/api/v1/survey-responses/', data=survey_response, format='json')
...
def test_no_deleted_question_in_survey_response(api_admin, api_user, survey_active):
    admin_got = api_admin.post('/api/v1/surveys/', data=survey_active)
    ...
    responses = [response['question_title'] for response in user_got['responses']]
