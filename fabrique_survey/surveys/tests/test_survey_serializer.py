...
def test_cant_update_survey_start_date(api_admin, survey_active):
    ...
    api_admin.put(f'/api/v1/surveys/{got["pk"]}/', data=survey_active, expected_status_code=400)


def test_cant_add_survey_with_invalid_end_date(api_admin, survey_invalid_end_date):
    api_admin.post('/api/v1/surveys/', data=survey_invalid_end_date, expected_status_code=400)


def test_cant_add_survey_without_questions(api_admin, survey_active):
    survey_active['questions'] = []
    api_admin.post('/api/v1/surveys/', data=survey_active, expected_status_code=400)
